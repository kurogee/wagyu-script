...
| split 変数名1 変数名2 区切り文字; | ``変数名2``を``区切り文字``で分割し、``変数名1``に代入します。 |
...
| 関数の形 | 実行内容 |
| --- | --- |
| replace 変数名1 変数名2 置換前 置換後 値; | ``変数名2``の文字列から``置換前``を``置換後``に``値``回置換し、``変数名1``に代入します。 |
| addbeg 変数名1 変数名2; | ``変数名2``の文字列を``変数名1``の文字列の先頭に追加します。 |
| addend 変数名1 変数名2; | ``変数名2``の文字列を``変数名1``の文字列の末尾に追加します。 |
| include 変数名1 変数名2 値; | ``変数名2``の文字列に``値``が含まれているかどうかを調べ、含まれている場合はtrue、含まれていない場合はfalseを``変数名1``に代入します。 |
| substr 変数名1 変数名2 開始位置 長さ; | ``変数名2``の文字列から``開始位置``から``長さ``分の部分文字列を取り出し、``変数名1``に代入します。 |
...
| calc add 変数名1 変数名2 (年 月 日); | ``変数名1``に``変数名2``(日付形式をフォーマット済み)に``年`` ``月`` ``日``を加算した日付を代入します。``(年 月 日)``は配列で指定し、省略する場合は0を指定します。 |
| calc sub 変数名1 変数名2 (年 月 日); | ``変数名1``に``変数名2``(日付形式をフォーマット済み)から``年`` ``月`` ``日``を減算した日付を代入します。``(年 月 日)``は配列で指定し、省略する場合は0を指定します。 |
